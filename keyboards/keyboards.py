...
# Инициализируем конструктор для клавиатуры главного меню
...
# Создаем inline-кнопки в разделе "год"
...
# Создаем объект inline-клавиатуры связанный с годами фильма
years_films = InlineKeyboardMarkup(inline_keyboard=[[years_1], [years_2], [years_3], [years_4], [pass_1], [back_1]])

# Создаем inline-кнопки в разделе "жанр"
...
# Создаем объект inline-клавиатуры связанный с рейтингом
rating_films = InlineKeyboardMarkup(inline_keyboard=[[high_8], [good], [average], [pass_3], [back_3]])

# Создаем inline-кнопки в разделе "время просмотра"
...
# Создаем объект inline-клавиатуры связанный с временем просмотра
time_films = InlineKeyboardMarkup(inline_keyboard=[[short], [t_average], [long], [very_long], [pass_4], [back_4]])

# Создаем inline-кнопки в разделе "показ результатов", то есть когда появляется список по критериям
...
# Создаем объект inline-клавиатуры связанный с сортировкой
...
# Создаем inline-клавиатуру для раздела "Фильмы по рейтингу"
...
# Создаем объект inline-клавиатуры связанной с "топом фильмов по рейтингу"
top_ratings_films = InlineKeyboardMarkup(inline_keyboard=[[top_back], [top_choice], [top_continue], [main_menu]])

# Создаем inline-кнопки для раздела "Случайный фильм из списка"
...
# Создаем inline-клавиатуру для раздела "Случайный фильм из избранного"
